...
public class Inputter {
    ...
    /** @param message prompt @return entered trimmed text, possibly blank */
    ...
    public Double readPrice(String message, boolean allowBlank) {
        while (true) {
            String text = readLine(message);
            if (allowBlank && text.isEmpty()) return null;
            try {
                double value = Double.parseDouble(text);
                if (value > 0) return value;
            } catch (NumberFormatException exception) {
                /* Common message below. */
            }
            System.out.println("Price must be a positive number.");
        }
    }

    /** @param message prompt @return true for Y, false for N */
