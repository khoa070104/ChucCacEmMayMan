...
public class DoctorHash {
    ...
    public boolean updateDoctor(Doctor doctor) throws Exception {
        ...
        if (!doctor.getSpecialization().isEmpty()) current.setSpecialization(doctor.getSpecialization());
    ...
    public boolean deleteDoctor(Doctor doctor) throws Exception {
        if (doctors.isEmpty()) throw new Exception("Database does not exist.");
        if (doctor == null || doctor.getCode() == null || doctor.getCode().trim().isEmpty()) throw new Exception("Data does not exist.");
        if (doctors.remove(doctor.getCode()) == null) throw new Exception("Doctor code does not exist.");
        return true;
    }
    ...
    public Map<String, Doctor> searchDoctor(String input) throws Exception {
        ...
        for (Doctor doctor : doctors.values()) {
            if (doctor.getCode().toLowerCase().contains(key) || doctor.getName().toLowerCase().contains(key)
                    || doctor.getSpecialization().toLowerCase().contains(key)) result.put(doctor.getCode(), doctor);
        }
        return result;
    }
    ...
    private void checkDoctorData(Doctor doctor) throws Exception {
        if (doctor == null || doctor.getCode() == null || doctor.getCode().trim().isEmpty()
                || doctor.getName() == null || doctor.getSpecialization() == null) throw new Exception("Data does not exist.");
        if (doctor.getAvailability() < 0) throw new Exception("Availability must be greater than or equal to zero.");
    }
}
