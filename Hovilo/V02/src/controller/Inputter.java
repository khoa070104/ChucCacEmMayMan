...
public class Inputter {
    ...
    /** @param message prompt @return entered line */
    ...
    public int readAvailability(String message, boolean allowBlank) {
        while (true) {
            String value = readLine(message);
            if (allowBlank && value.isEmpty()) return -1;
            try {
    ...
    /** @return entered integer menu choice */
    public int readChoice() {
        while (true) {
            try {
                return Integer.parseInt(readLine("Please choose: "));
            } catch (NumberFormatException exception) {
