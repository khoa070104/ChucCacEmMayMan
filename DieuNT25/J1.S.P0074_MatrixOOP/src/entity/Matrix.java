...
public class Matrix {
    ...
    //Cô bảo viết thêm hàm tính giá trị tuyệt đối của ma trận
    public Matrix absolute() throws Exception {
        int[][] dataResult = new int[rows][cols];
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                // Sử dụng Math.abs để lấy giá trị tuyệt đối
                dataResult[i][j] = Math.abs(this.data[i][j]);
            }
        }
        return new Matrix(dataResult);
    }
