...
public class StudentDAO {
    ...
    public StudentDAO(String fileName) { fileManager = new FileManager(fileName); }
    ...
    public void loadDataFromFile() throws IOException {
        ...
        for (String line : fileManager.readDataFromFile()) {
            ...
            try {
                Student student = new Student(fields[0].trim().toUpperCase(), fields[1].trim(),
                        fields[2].trim(), Double.parseDouble(fields[3].trim()));
                if (DataValidation.isStudentId(student.getId())
                        ...
                        && DataValidation.isGpa(student.getGpa()) && findById(student.getId()) == null) {
    ...
    public List<Student> findByMajor(String major) {
        return students.stream().filter(s -> s.getMajor().equalsIgnoreCase(major.trim()))
                .collect(Collectors.toList());
    }
    public List<Student> sortedByGpa() {
        return students.stream().sorted(Comparator.comparingDouble(Student::getGpa)
                .thenComparing(Student::getId)).collect(Collectors.toList());
    }
    public void saveDataToFile() throws IOException {
        fileManager.saveDataToFile(students.stream().map(Student::toString).collect(Collectors.toList()));
    }
}
